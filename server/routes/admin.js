...
router.patch("/doctors/:id/approve", requireAuth(["admin"]), async (req, res) => {
  ...
  const body = note
    ...
    : "You can now log in and verify cases.";
  await notify(req.params.id, "doctor_approved", "Doctor account approved", body, "/login");
  res.json({ ok: true });
});
...
router.patch("/doctors/:id/reject", requireAuth(["admin"]), async (req, res) => {
  ...
  await notify(req.params.id, "doctor_rejected", "Doctor application rejected", note, null);
  res.json({ ok: true });
});
