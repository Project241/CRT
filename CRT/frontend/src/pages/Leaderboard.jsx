...
const CATEGORIES = [
  { id: "streakScore", label: "Overall", icon: "✦", desc: "Streak strength score" },
  { id: "avgScore",    label: "Avg score", icon: "🎯", desc: "Highest average per attempt (3+ attempts)" },
  { id: "streakDays",  label: "Streak", icon: "🔥", desc: "Longest day-streak" },
  { id: "attempts",    label: "Cases solved", icon: "📚", desc: "Most cases attempted" },
  { id: "specialty",   label: "By specialty", icon: "🏥", desc: "Top scorer in a chosen specialty" },
];
  ...
  return (
    ...
    >
      ...
      <div className="muted small" style={{ marginTop: 4 }}>
        {metric?.sub || `🔥 ${entry.currentStreak}-day streak · Best ${entry.bestStreak}`}
      </div>
      {entry.topSpecialty && (
        <div
          style={{
...
export default function Leaderboard() {
  ...
  function rowMetric(row) {
    ...
    if (category === "streakDays")  return { value: row.bestStreak, sub: `🔥 ${row.currentStreak} now` };
