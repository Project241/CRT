...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      api.get("/api/verify/health").catch(() => null),
      api.get("/api/verify/triage").catch(() => ({ cases: [], mySpecialty: null, quorum: 3 })),
      api.get("/api/verify/touched").catch(() => ({ cases: [] })),
      api.get("/api/discussions/delete-requests").catch(() => ({ requests: [] })),
      api.get("/api/lounge?limit=4").catch(() => ({ messages: [] })),
    ]).then(([h, t, tt, dr, lg]) => {
      if (!alive) return;
      setHealth(h);
      setTriage(t);
                    ...
                    <TriageItem key={c.id} c={c}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div className="triage-head">
                          ...
                          {c.report_count > 0 && (
                            <span className="quorum-pill reports">⚑ {c.report_count} report{c.report_count > 1 ? "s" : ""}</span>
                          )}
                          {c.tips_quorum && (
                            <span className="quorum-pill tips">★ Tips quorum</span>
                          )}
                        </div>
                ...
                <div className="touched-list">
                  {touched.map((c) => (
                    <Link key={c.id} href={`/case/${c.id}`} className="touched-row">
                      ...
                      <span className={`badge ${c.my_action === "verify" ? "badge-success" : "badge-warning"}`}>
